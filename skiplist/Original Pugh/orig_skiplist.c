...
#ifdef allowDuplicates
    void insert(register list l, register keyType key, register valueType value)
#else
    bool insert(l, key, value)
#endif
{
    register int k;
    ...
    if (q -> key == key) {
        //update value, but not insert duplicate key.
        q -> value = value;
        return (false);
    };
    #endif
...
bool delete(register list l, register keyType key)
{
    register int k, m;
    node update[MaxNumberOfLevels];
    register node p, q;
...
bool search(register list l, register keyType key, valueType* valuePointer)
{
    register int k;
    register node p, q;
    p = l -> header;
